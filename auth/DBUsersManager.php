...
/*
 * Класс для работы с БД с таблицей Пользователи
 */
...
class DBUsersManager
{
    ...
    function getUserData($username){
        $safe_username = $this->db_handle->real_escape_string(htmlspecialchars($username));
        $res = $this->db_handle->query("SELECT username, password, first_name, last_name, patronymic, birth_date, locations, marital_status, education, experience, contacts, other, photo_path, user_lang_code FROM users WHERE username='$safe_username'");
        if ($res->num_rows) {
